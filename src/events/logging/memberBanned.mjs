...
export default new BotEvent(
    ...
    async (bot, ...args) => {
        ...
        if (ban.guild) {
            log.debug(`Handling ban log event on guild of ID ${ban.guild?.id}...`);
            ...
            if (system) {
                if (system.logs.enabled && (system.logs.actions.ban)) {
                    ...
                    log.warn(`Logs for bans not enabled in guild '${ban.guild?.name}' (${ban.guild?.id})`);
                };
            } else {
            ...
            log.error(`Banned user of ID ${ban.user?.id} not from a guild`);
            return;
        };
    });